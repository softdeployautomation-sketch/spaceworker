...
2. The EXE, even before any key is entered, can compute its own machine id locally (`lib/machine-id.ts`'s port already exists) and **display it on screen** — "Your Device ID: `XXXX` — copy this into your account's Licenses page to activate."
3. The buyer pastes that device id into a "claim" form (see below — admin-run first, self-service second). The claim endpoint:
