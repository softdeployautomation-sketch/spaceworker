...
| OWN-7 | **Wake**: Power → **Wake** against a powered-off machine | console | **PARTIALLY CLOSED 2026-09-26 — see BUILD-4.** Keep-awake is built, deployed, and live-verified working on `Sc` (`powercfg /requests` shows a real SYSTEM entry while held, clears on Stop). The peer-relay Wake path is also built + deployed but **cannot be exercised** — `Sc` has no same-subnet peer today (measured: fleet spans four different subnets). Still open, correctly: get a second Windows device onto `Sc`'s LAN and re-test Wake specifically |
| OWN-8 | **TASK_125** mint a public link **with a guide PDF attached**, download + run it: the zip contains `guide.pdf` and it **opens right after install** | Vantra connect card → naming card → *Install guide* | **The server half is DONE + PROVEN 2026-09-26** (BUILD-9 / §8): a real minted zip was downloaded and listed — `guide.pdf` is inside the launcher folder, byte-exact, `%PDF-` magic. What remains is only the **Windows** half: that the launcher **opens** the guide right after install. **Fold it into OWN-5** when that click-through is finally done — same minted link, one extra assertion |
...
| BUILD-2 | **B10 / TASK_120 — DEFERRED (owner, 2026-09-26)**: *"i dont think the clone is necessary for now, we can close that and allow michael fix it himself whenever he got time."* | ship `clone-native-host.exe` + `install-registry.ps1` in `engine-dist` **and** `ROLE_ARTIFACTS.source`; register the native host; write the **registry `update_url`** (never a policy); persist `DeviceSetupRun`; per-section activity UI; one button → running clone | **not urgent, not assigned.** Michael's own track whenever he has time. Live Browser Clone capture stays unavailable until this lands — accepted as fine for now |
...
| ~~BUILD-5~~ | **G1 / TASK_105 — DONE + DEPLOYED 2026-09-26 (Cline, `agent/task-105-resource-governor` → `main` `7685087`)** | `lib/resource-governor.ts` (one admission seam, `requestSlot`/durable `GovernorQueueEntry` queue), wired into clone sessions + the pooled hosted PC, admin panel (cap/live/queued per feature + all 6 thresholds), `governor-sweep.timer` (1 min) | Independently re-verified (not just Cline's own report): 26/26 tests re-run clean, `tsc`/`next build`/eslint all clean, merged, deployed, drift check empty. Sweep manually triggered live — real host pressure returned (`ramUsedPct: 19.3%`, 8 CPUs), auth correct, `enabled: false` (safe default, byte-identical behavior until an admin turns it on). Admin `/api/admin/governor` verified live with a real admin session — all 7 features report real live/queued counts (`vantraLinks: live=2` is real production data) |
| BUILD-6 + BUILD-7 | **TASK_124 (bundles F1/F2 + B8-4) — ASSIGNED TO CLINE 2026-09-26** | engine Chrome KDF salt (`saltysalt`, not `peanuts`) + the unhandled 16-byte cookie prefix; retire `self_only` at `lib/clone-hosts.ts:76,153`, `components/device-console.tsx:86,1924`, `lib/clone-setup.ts:125` if provably dead | off the critical path but genuine bugs; see `TASK_124_CRYPTO_FIX_AND_SELF_ONLY_DEADCODE.md` for the full writeup (exact file/line/fix for both) |
...
| BUILD-9 | **TASK_125 / OOB-13 follow-up — the public ZIP's install-guide PDF, the half TASK_121 left out (owner report 2026-09-26: *"during the zip creation in vantra, there was a place user can add a pdf that auto opens after the user opens the file … but right now its not in spaceworker, i think that was missing"*)** — branch `agent/task-125-zip-guide-pdf` in BOTH repos | TASK_121 wired the three renameable **names** through the `sw-` install-link contract and stopped there; the optional **guide PDF** (Vantra Task 77/78 "FIX 5") was never added — no file input in the naming card, no field on `InstallerNames`, and `parseInstaller`/`callZipGenerator` on the `sw-` path never received `pdf*`. Extends the SAME frozen `installer` block with `pdf`/`pdfName`/`pdfDelaySec` (additive; a no-PDF request is byte-identical, and there is **no schema change**). Loud gate on SpaceWorker's route (400/413), total never-throwing gate on Vantra's, bytes forwarded and **never stored** | `TASK_125_PUBLIC_ZIP_GUIDE_PDF.md`. **DEPLOYED 2026-09-26 + LIVE-VERIFIED (Cline)** — see `TASK_125_PUBLIC_ZIP_GUIDE_PDF.md` §8. Both repos deployed via CI `workflow_dispatch` **from the branch ref** (Vantra first); pre-flight migration audit confirmed nothing pending; the running artifacts were proven to be built from the TASK_125 source (deployed source-map evidence, not just "the deploy said success"), and a real mint's zip was downloaded and listed: `T125Proof.lnk` + `T125Proof/Launcher.exe` + `T125Proof/agent.bin` + **`T125Proof/T125 Guide.pdf`** (614 bytes, magic `%PDF-`). **A blocking bug was found and fixed during this deploy (§8): `spaceworker.top` set no `client_max_body_size`, so nginx's built-in 1 MB default 413'd every realistic PDF before the app could validate it** — fixed to 32M on both hops, live-verified with a 2 MB POST (401 from the app, not nginx 413), and the repo's reference copy `deploy/nginx-spaceworker.conf` was corrected too. ✅ **MERGED + FINAL 2026-09-26.** Vantra fast-forwarded to `9d2f086`; SpaceWorker took merge commit `cedfc67` (main had diverged with TASK_94). Both branches are verified ancestors of their `main`. This was not theoretical: while the feature existed only on the branch ref, a **15:14Z deploy of `main` silently reverted it** — the route's traced chunk went from 7 TASK_125 markers to **0** — so the merge was followed by a deploy **from `main`**, re-verified the same way (7 markers, fresh chunk mtime). See §8's "the revert" write-up. Only the Windows auto-open half remains with the owner (OWN-8) |
...
| GATE-4 | Old backlog the owner still owns | `TASK_93` Vantra plugin (live) · ~~`TASK_94` Telegram approvals~~ **DONE + DEPLOYED 2026-09-26 (Claude)** — real end-to-end verified live on production (approve executes exactly once, real audit trail, garbage tokens 403, re-taps 409, reject never runs anything; a genuine race-condition bug in the pre-existing "free the proposal back up on failure" recovery was found by the new test suite and fixed in both the new code and the original web route) · ~~`TASK_95` devices parity~~ **DONE — confirmed already built across TASK_103/104/108/114/119/122/123, doc was just stale** · ~~`TASK_99` module store~~ **DONE 2026-09-26** · ~~`TASK_100` repositioning~~ **DONE 2026-09-26** · `TASK_101` account/staff parity — **ASSIGNED TO CLINE 2026-09-26** · `TASK_TIER1` trial. **Pre-existing — not clone-pipeline fallout** |
...
- 2026-09-26 — **BUILD-4 / TASK_123 (B12) DONE + DEPLOYED.** Both PATH A
  ...
  fixed only by that live test — see BUILD-4 row). Also: TASK_100 MK5 (nav
  cleanup — Advanced Search + Licenses out of the dashboard nav, folded into
  Extract/Settings respectively) done + deployed; MK1-4 (the actual hero/
  copy rewrite) is now ungated (P1/P2/P3 flagship — Vantra plugin, browser
  clone, WoL/keep-awake — are all live) but not yet started.
