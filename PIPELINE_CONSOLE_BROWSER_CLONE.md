...
| OWN-8 | **TASK_125** mint a public link **with a guide PDF attached**, download + run it: the zip contains `guide.pdf` and it **opens right after install** | Vantra connect card → naming card → *Install guide* | deferred with the deploy (see BUILD-9). The server half is checkable with `curl` + an archive listing; this row is the part that needs a real Windows machine. **Fold it into OWN-5** when that click-through is finally done — same minted link, one extra assertion |
