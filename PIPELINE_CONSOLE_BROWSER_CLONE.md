...
| B7 | `TASK_117_HOSTED_POOL_PROVISIONING.md` | **Provision the hosted PC pool** — the machine our clones actually run on. `Device.deviceKind = "hosted"` is read in exactly one place and **written nowhere**, so the pool is permanently 0 and `hostedPoolSize` is a dial nothing acts on. Scoped: D1 Windows→host profile-portability spike (the real risk), D2 hosted-PC identity under a **system tenant**, D3 parcel + relay transport, D4 stream into the app, D5 stop offering "Set up as clone host" to customers, D6 TTL/governor. Guardrail: **a customer device is never clone infrastructure** | B4, B4-fu | **SCOPED 2026-09-24** — needs Q1 (Linux container on our VPS vs dedicated Windows VM), Q2 (headless hosted browser OK?), Q3 (drop the customer-facing button?) |
