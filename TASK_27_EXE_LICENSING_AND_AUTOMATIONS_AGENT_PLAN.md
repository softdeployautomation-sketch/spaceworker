...
### The Automation-enabled EXE's agent connection is a server-side proxy, not a direct Channelry call (added 2026-09-14)

The automation-enabled EXE is "the only one of the four with any network dependency for its core function" (per this doc's own line above) — but that network call must NOT be the EXE calling Channelry's `/external/ai-chat` directly. `CHANNELRY_AI_API_KEY` is a **single pooled secret for the entire `spaceworker` client_id**, with one shared $50/day cap covering every SpaceWorker user combined (web AND desktop). Shipping it inside a distributable EXE — thousands of copies on buyers' own machines — means anyone can extract it (it's not real DRM, same accepted tradeoff as the license-validation secret) and exhaust the ENTIRE pooled cap for every other customer, web and desktop alike. It must never leave the server, full stop — same fail-closed discipline as every other secret in this app.

The same reasoning rules out running `runAgentTurn`'s cap/credit logic locally too: `aiDailyCapHundredthsCent`/`aiCreditBalanceHundredthsCent` represent real money already paid, stored on the shared Postgres `User` row — enforcing that from code running on the buyer's own machine (editable by that same buyer) isn't trustworthy.

**The fix**: a new production-only endpoint, `POST /api/exe-agent/chat`, authenticated by the license key itself instead of a session cookie — body `{ licenseKey, email, message }`.
- Reuse `decodeLicenseKey`/`verifyLicenseKey` (`lib/exe-license.ts`, already live for issuance and the offline validator) to check the key is genuine and unexpired, server-side.
- Look up `ExeLicense.licenseKey` → `userId`, and confirm `ExeLicense.product` is the automation-enabled tier (the only one of the four entitled to agent access — a Lead/Mailer/Combined-only license must be rejected here).
- Then call the exact same `runAgentTurn({ userId, message })` (`lib/agent.ts`) the web chat widget already calls. Zero duplicated cap/credit/tool-calling/Channelry logic — this is a second, differently-authenticated entry point into code that already exists, not a new implementation.
- Because `ExeLicense.userId` is a real `User` row created at purchase, that buyer already has Task 40's normal free daily allowance (`aiDailyCapHundredthsCent`, default $2/day) with zero extra work — the credit balance (Task 43) is purely the overflow-past-that-allowance path, identical to the web flow.
- The EXE's local build's agent-chat UI is therefore a **thin network client** of this one endpoint, not a local reimplementation of the agent — it genuinely needs internet to work, which is the accepted exception this doc already carves out.
- Trust model: anyone holding a valid automation-enabled license key can call this endpoint — that's by design, the same "possession of a valid HMAC key is proof of entitlement" model the offline validator already accepts. No session, no additional auth needed or wanted.

See Task 43 (`TASK_43_AI_CREDIT_TOPUPS.md`) for how a desktop buyer actually purchases more credit balance without ever needing a web dashboard login — it hangs off this same license-key identity.
