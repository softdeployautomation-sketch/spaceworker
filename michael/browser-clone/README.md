...
## Test evidence
- Self-test suite (`tests/Test-Roundtrip.ps1`): GCM seal/open roundtrip, tamper rejection, wrong-key rejection, §2 file-list detection on a synthetic profile, container-format constants — designed to run on Windows PowerShell 5.1 and pwsh 7 (hosted on the lab Windows VM for the CNG path; native-PS capture/restore verified against a synthetic Chrome-shaped profile tree).
