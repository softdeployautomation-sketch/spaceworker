...
export function Dock() {
  ...
  return (
    <nav
      aria-label="Dock"
      className="fixed bottom-4 left-1/2 z-30 hidden -translate-x-1/2 items-end gap-2 rounded-[20px] border border-[#29314a] bg-[#0d1320]/80 px-3 py-2 shadow-[0_24px_60px_-18px_rgba(0,0,0,0.8)] backdrop-blur-xl md:flex"
    >
      ...
      <div aria-hidden className="mx-1 w-px self-stretch bg-[#29314a]" />
...
function DockIcon({ href, label, icon: Icon, active }: NavItem) {
  return (
    ...
    >
      ...
      <span
        className={cn(
          "flex h-11 w-11 items-center justify-center rounded-[13px] border bg-gradient-to-b from-[#1c2333] to-[#161b26] shadow-[0_10px_24px_-10px_rgba(0,0,0,0.6)] transition-transform duration-150 ease-out group-hover:-translate-y-2",
          active
      ...
      >
        <Icon
          className={cn(
            ...
            active
              ? "text-[#a5b4fc]"
              : "text-[#8b93a7] group-hover:text-[#a5b4fc]",
          )}
          strokeWidth={1.8}
          aria-hidden="true"
        />
      ...
      <span
        className={cn(
          ...
          active ? "bg-[#818cf8]" : "bg-transparent group-hover:bg-[#3f4759]",
        )}
        aria-hidden="true"
      />
    </Link>
