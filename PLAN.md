...
### Phase 1 scope — CONFIRMED with the user 2026-09-03 (supersedes "two real decisions" below, kept for the reasoning trail)

**Architecture — no per-user VMs, ever, for this stage.** Confirmed explicitly: the cost of per-user VMs was considered and rejected as too expensive for this stage. Phase 1 is **one shared box (the existing VPS), multiple Chrome/Chromium processes, one persistent profile directory per user** — literally "a single browser, each user gets a profile, the way Chrome does" (the user's own words) — i.e. exactly Task 6's existing profile-directory mechanism (`/mnt/browser-profiles/{userId}/`), just launched non-headless (or headless-with-a-virtual-display, since it needs to render pixels to stream) instead of fully headless, and reused as a second consumer alongside Task 2's scraper. **No new profile-storage design needed — Task 6 already built the right foundation.**

**Concurrency target: 2–3 simultaneous interactive sessions on the current VPS, not unlimited.** This is a deliberate Phase 1 cap, not an aspiration — matches the "stage 1 before more funds, bigger build later" framing. Capacity planning (§ below) should be sized around this number specifically, not "as many as fit."

**IP/proxy model — CONFIRMED, and simpler/cheaper than the earlier paid-residential-proxy research assumed:**
1. **Free default tier**: SpaceWorker provides its own VPN-style exit routes to a small number of popular locations — **not** a paid per-GB residential/mobile proxy service (that cost model is explicitly deferred, see point 3). The practical Phase 1 shape: self-hosted WireGuard (or OpenVPN) exit nodes on a handful of small, cheap VPS instances in a few popular countries (e.g. US, UK, maybe 1–2 more) — genuinely cheap (a few small droplets, not per-GB billing), and each user's browser process routes its traffic through whichever exit node matches their chosen location. **Not yet decided**: exact node count/locations/hosting provider for these exit nodes — small follow-up decision, not blocking the rest of this scope.
2. **BYO (bring your own)**: users can enter their own proxy/VPN credentials (host, port, protocol, auth) per profile — for users who already have a residential/mobile proxy account elsewhere (from any of the providers researched above) and want to use it instead of the free routes. This needs a per-profile settings field + validation (test-connect before trusting it, same discipline as Task 4's mailbox `test-connection` flow), not a new proxy-provider integration on SpaceWorker's side — SpaceWorker is just a pass-through configuration here.
3. **Future, explicitly NOT this stage**: a paid premium-IP tier SpaceWorker itself sells (sourcing a "good IP" worth charging for) — deferred until real usage/revenue justifies picking and integrating a paid proxy provider. Do not build toward this yet; the free + BYO model above is the complete Phase 1 scope.

**Still open — streaming technology** (the one piece not yet decided): candidates worth a real spike/comparison before committing — Neko (`n.eko`, self-hosted, WebRTC-based, purpose-built for "stream one browser with input forwarding," Docker-friendly which fits a one-container-per-active-user model cleanly) vs. Kasm Workspaces (broader open-source remote-browser-isolation platform, more moving parts than likely needed for a 2–3 user cap) vs. a lighter custom build on Chrome's own CDP screencast protocol. **Recommendation, not yet confirmed with the user: Neko** — smallest footprint of the three for this concurrency target, purpose-built for exactly this problem rather than a general-purpose platform sized for much larger deployments.

**Control granularity — three independent levels, all confirmed as required:**
1. **Whole SpaceWorker app off** — already exists (`spaceworker.service` in Vantra's shared instaweb Services panel, per the earlier V6 work).
2. **Whole browser-streaming subsystem off** (all interactive sessions killed, headless extraction/Task 2 unaffected, rest of the app unaffected) — this needs to be **its own separate systemd service** (e.g. `spaceworker-browser.service`), distinct from both `spaceworker.service` (the main app) and the eventual `spaceworker-worker.service` (Task 2's headless extraction) — a third independently-controllable process, following the exact same pattern already locked in for Task 2 above. Add to Vantra's shared `CONTROLLABLE_UNITS` allowlist once deployed, same mechanism already used for the other two.
3. **Individual user's session off** — this is an **application-level** control, not a systemd-level one (systemd only knows about the whole subsystem, not which Chrome process belongs to which user) — needs an admin-panel action inside SpaceWorker itself that finds and kills the specific user's browser/streaming process without affecting other concurrent users. This is new, SpaceWorker-side work (not something Vantra's shared services panel can do, since it operates at the systemd-unit level only) — likely a small in-app process registry (`userId` → PID or container ID) the admin UI reads from and can send a kill/stop action against.

**Browser-panel UI — confirmed requirement:** the interactive browser view needs an **IP checker** showing the session's *current* effective IP (a real, live check — e.g. calling an "what's my IP" endpoint from *inside* the streamed browser session itself or from the server process routing its traffic, not a cached/assumed value, so a broken exit-node route is visibly caught rather than silently trusted), plus **inline controls to switch** which free-location route the session uses and to **add/edit a BYO proxy** — both reachable from the same panel the user is already looking at, not a separate settings page. "For now" (the user's words) — keep this UI simple for Phase 1 (a location dropdown + a BYO credentials form + the live IP readout), not a polished multi-step wizard.

### Explicitly not started

No Prisma model, no route, no UI sketch yet — implementation hasn't begun. What's now confirmed (architecture, concurrency cap, IP model, browser-panel IP-checker/switcher UI) is enough to write a real task spec; what's still open (exact exit-node locations/count, final streaming-tech sign-off on the Neko recommendation) are small follow-ups, not blockers to starting. Once those are nodded through, this becomes its own task spec (likely `TASK_07_PRIVATE_BROWSER_PHASE1.md`), sized and assigned separately from Task 2/3 (Cline/Michael's current backlog) — not bundled into either.
